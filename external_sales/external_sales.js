...
const create_external_sales = (request, response) => {
    const id_inventario = parseInt(request.params.pos_id)
    const { products } = request.body

    pool.query(`select *
        from bodegas
            where id_inventario = $1`,[id_inventario],
            (error_bodegas, bodegas_result) => {
                if (error_bodegas) {
                    response.status(500).send('Something went wrong were sorry')
                } else {  
                    let bodegas = bodegas_result.rows[0];
                    if (request.body.external_sale_id == undefined) {
                        ...
                        created_at, created_by) values($1,$2,$3,$4,$5,now() at time zone 'America/Guatemala',$6) RETURNING id_sale`,
                            [
                                bodegas.id_bodega, massive_sale_id, customer_id, total_sale, total_discount,
                                created_by
                            ],
                            (error, sales_result) => {
                                if (error) {
                                    ...
                                    if (sales_result.rows.length > 0) {
                                        products.map((item) => {
                                            axios({
                                                method: 'post',
                                                url: bodegas.base_url + 'bodega/' + bodegas.id_bodega + '/out',
                                                data: {
                                  ...
                                  values($1,$2,$3,$4,$5,$6,$7,$8,$9,now() at time zone 'America/Guatemala',$10)`,
                                                        [sales_result.rows[0].id_sale, bodegas.id_bodega, item.product_code, item.quantity,
